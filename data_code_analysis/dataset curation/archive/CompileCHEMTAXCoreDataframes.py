...
"""
This code is the exact same as 'CompileMergedCoreDataframes', but takes the 
mean when grouping instead of the median. This is done specifically because the 
CHEMTAX data can be summarized using the mean rather than the median in 
order to maintain a summation to 1 (100%). The datasets produced from this code
are only used to acquire the mean community proportions of the 5 major phyto 
groups from CHEMTAX. Those proportions are then used to plot yearly mean 
community proportions directly, as well as used to determine group-specific Chla 
concentrations via multiplying the mean group proportions by the median Chla 
...
# def extract_relevant_params(df):
#     paramlist = ['Year',
#                  'Region',
#                  'IORegion',
#                  'NSRegion',
...
#                  'PrasinophyteBiomass']
#     filtered_df = df[paramlist].reset_index()
#     return filtered_df
...
# dfbiosrf = dfbiosrf1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event']).mean().reset_index()
# dfhydsrf = dfhydsrf1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event']).mean().reset_index()
...
# current_directory = Path.cwd()
# absolute_path = Path("local data/")
# filename = Path("PALLTER_EventLevel_SurfaceAvgCHEMTAXCoreDataframe.csv")
...
# dfbiosrf = dfbiosrf1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event']).mean().reset_index()
# dfbiosrf = dfbiosrf.groupby(['Year','RoundedGridLine','RoundedGridStation']).mean().reset_index()

# dfhydsrf = dfhydsrf1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event']).mean().reset_index()
# dfhydsrf = dfhydsrf.groupby(['Year','RoundedGridLine','RoundedGridStation']).mean().reset_index()
...
# current_directory = Path.cwd()
# absolute_path = Path("local data/")
# filename = Path("PALLTER_StationLevel_SurfaceAvgCHEMTAXCoreDataframe.csv")
# savepath = str(current_directory / absolute_path / filename)
...
# # =============================================================================
# #reduce base dataframes to event-level
# dfbiodep = dfbiodep1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event', 'Depth']).mean().reset_index()
# dfhyddep = dfhyddep1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event', 'Depth']).mean().reset_index()
...
# current_directory = Path.cwd()
# absolute_path = Path("local data/")
# filename = Path("PALLTER_EventLevel_DepthAvgCHEMTAXCoreDataframe.csv")
# savepath = str(current_directory / absolute_path / filename)
...
# #reduce base dataframes to station-level
# dfbiodep = dfbiodep1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event', 'Depth']).mean().reset_index()
# dfbiodep = dfbiodep.groupby(['Year','RoundedGridLine','RoundedGridStation', 'Depth']).mean().reset_index()

# dfhyddep = dfhyddep1.groupby(['Year','RoundedGridLine','RoundedGridStation','Event', 'Depth']).mean().reset_index()
# dfhyddep = dfhyddep.groupby(['Year','RoundedGridLine','RoundedGridStation', 'Depth']).mean().reset_index()
...
# current_directory = Path.cwd()
# absolute_path = Path("local data/")
# filename = Path("PALLTER_StationLevel_DepthAvgCHEMTAXCoreDataframe.csv")
# savepath = str(current_directory / absolute_path / filename)
