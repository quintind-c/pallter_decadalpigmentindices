...
if c == None:
    points = ax1.scatter(x=x, y=y, c=None, cmap=cmap, data=data, marker='o',
                         s=markersize, color='grey', linewidths=0.75, edgecolor='k', zorder=3)
else:
...
# ax1.set_xlabel('Max $\mathregular{N^{2}}$ ($\mathregular{s^{-1}}$$\mathregular{x10^{-3}}$)', fontweight='normal', size=12)
ax1.set_xlabel('Max $\mathregular{N^{2}}$ ($\mathregular{s^{-1}}$)', fontweight='normal', size=12, labelpad=1.5)
ax1.set_ylabel('PPC:Chla', fontweight='normal', size=12)
ax1.tick_params('x', labelbottom=True)
ax1.tick_params('both', labelsize=8)
ax1.set_ylim(0.15, 0.725)
...
filename = Path("Fig5g_PPCvMaxN2.pdf")
