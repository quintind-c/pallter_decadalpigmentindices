...
color_contour = ax1.contourf(sal_matrix.columns.values, sal_matrix.index.values, sal_matrix.values, 
             levels=21, cmap='coolwarm') #YlGnBu
line_contour = ax1.contour(sal_matrix.columns.values, sal_matrix.index.values, sal_matrix.values, 
...
ticks = np.arange(32, 35, 0.5)
cbar = fig.colorbar(color_contour, ax=ax1, orientation='vertical', pad=0.025, ticks=ticks)
cbar.set_label('Salinity (g $\mathregular{kg^{-1}}$)', fontweight='normal', fontsize=12)
...
# #plot WWUpper points
# wwuline1, = ax1.plot(wwudata_filled['Year'], wwudata_filled['WWUpper'], marker='D', 
...
# #plot WWLower points
# wwlline1, = ax1.plot(wwldata_filled['Year'], wwldata_filled['WWLower'], marker='D', 
...
# ax1.legend(handles=[wwlline1], bbox_to_anchor=(0.185, -0.02), ncols=1, loc=4, fontsize=8, 
#            frameon=True, framealpha=1, borderpad=0.2, handletextpad=0.5)
# -----------------------------------------------------------------------------
...
filename = Path("Fig3e_SalCon.pdf")
