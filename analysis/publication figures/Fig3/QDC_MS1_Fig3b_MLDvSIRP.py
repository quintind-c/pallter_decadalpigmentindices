...
if c == None:
    points = ax1.scatter(x=x, y=y, c=None, cmap=cmap, data=data, 
                         s=markersize, color='Red', linewidths=0.5, edgecolor='black', zorder=3)
else:
...
ax1.set_xlabel('MLD (m)', fontweight='normal', size=12, labelpad=2)
# ax1.set_ylabel('$\mathregular{WW_{Upper}}$ (m)', fontweight='normal', size=12, labelpad=4)
...
ax1.tick_params(axis='y', which='major', colors='Red', length=4, width=1.05)
ax1.tick_params(axis='y', which='minor', colors='Red', length=2.5, width=1.05)
ax1.set_ylim(-90, 31)
# ax1.set_xlim(10, 75)
ax1.minorticks_on()
...
filename = Path("Fig3b_MLDvSIRP.pdf")
