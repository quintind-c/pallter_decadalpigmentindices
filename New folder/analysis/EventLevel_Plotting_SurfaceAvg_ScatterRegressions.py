...
df_EM = pd.read_csv(loadpath)

# #calculate yearly medians
# temp = remove_bad_years(df_EM)
...
dfD = df_EM[df_EM['Diatoms'] >= 0.9]
dfC = df_EM[df_EM['Cryptophytes'] >= 0.9]
dfM = df_EM[df_EM['MixedFlagellates'] >= 0.9]
...
c=None
cmap = 'viridis'
xscale = 'log' #linear, log, symlog
yscale = 'log' #linear, log, symlog
data = df_EM

# data = data[data['Year'] != 2009]

Dpoints = ax1.scatter(x=x, y=y, c=None, cmap=cmap, data=dfD, s=markersize, color='tab:green', zorder=3)
Cpoints = ax1.scatter(x=x, y=y, c=None, cmap=cmap, data=dfC, s=markersize, color='tab:blue', zorder=3)
Mpoints = ax1.scatter(x=x, y=y, c=None, cmap=cmap, data=dfM, s=markersize, color='tab:orange', zorder=3)
...
# # ^uncomment this section to plot a 1:1 line for comparison
# min_val = min(min(x), min(y))
# max_val = max(max(x), max(y))
# ax1.plot([0.01, 100], [0.01, 100], 'r--', color='grey', label='1:1 Line')
